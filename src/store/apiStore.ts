...
interface AppState {
  ...
  // Anticheat status
  anticheatStatus: any | null;
  loadAnticheatStatus: () => Promise<void>;
  ...
  loadClassesByStudent: (studentId: string) => Promise<void>;
...
export const useStore = create<AppState>((set, get) => ({
  ...
  login: async (email, password, role) => {
    try {
      ...
      if (role === 'instructor') {
        ...
        await get().loadClassesByStudent(response.user.id);
      }
    } catch (error) {
  ...
  loadClassesByStudent: async (studentId) => {
    try {
      set({ isLoading: true, error: null });

      const classes = await apiClient.getClassesByStudent(studentId);
  ...
  anticheatStatus: null,

  loadAnticheatStatus: async () => {
    try {
      const status = await apiClient.getAnticheatStatus();
      set({ anticheatStatus: status });
    } catch (error) {
      console.error('Error loading anticheat status:', error);
      // Don't set global error for this background task
    }
  },
}));
